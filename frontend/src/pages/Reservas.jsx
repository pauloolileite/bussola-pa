...
// Data de hoje (local) no formato aceito por <input type="date">.
function hojeISO() {
...
const INPUT_CLASS =
  'w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm outline-none ' +
  'bg-white cursor-pointer focus:border-blue-400 transition-colors'

// Formulário extraído para o escopo do módulo. Antes era redefinido a cada
// render do componente pai, o que remontava os inputs (perda de foco e estado).
function FormReserva({
  formData, setFormData, data, setData, horario, setHorario,
  onSubmit, onCancel, erroForm, clientes, passeios, guias, loading,
  onNovoCliente, minData,
}) {
  return (
  ...
  async function confirmarEditar() {
    ...
    try {
      const res = await api.patch(`/reservas/${reservaSelecionada.id}/`, {
        ...formEditar, data_reserva: dataEditar, horario_reserva: horarioEditar
      })
